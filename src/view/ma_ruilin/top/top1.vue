  ...
  <div class="top1">
    ...
    <span class="box">
      <div class="swich" style="margin-left:.1rem;">
        &nbsp;
        <span style="color:white;font-size:.3rem">{{fh}}</span> &nbsp;
        <van-icon class="d" name="eye" />
      </div>
      <div style="font-size:.3rem;color:#FFFFFF;margin-left:.26rem;">3333</div>
      <span class="box_o" style="background:white;border-radius:4px">
        <span>99</span>
        <span>99</span>
        <span>99</span>
      </span>
    ...
    <div
      style="width:100%;height:.3rem;color:#FFFFFF;text-align:center;font-size:.25rem;margin:.07rem 0 .13rem 0;"
    >{{bname}}</div>
    <div style="width:100%;height:.74rem;" class="sd">
      <div>
        <span
          style="width:100%;height:.3rem;background:#3D4046;text-align:center;line-height:.3rem;margin-left:3px"
        >待ち人数</span>
      ...
      <div>
        ...
        >待ち時間</span>
        <span style="text-align:center;font-size:.26rem;color:#FFFFFF">{{awitTime | timeFormat}}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      awitPerson: 300,
      Tnum: 300,
      bname: localStorage.getItem("name"),
      bgid: "777",
      awitTime: 99.99,
      fh: localStorage.getItem("fh") //受付番号
    };
  },
